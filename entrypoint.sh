...
echo "Configuring Tinyproxy deny filter..."
: > /app/tinyproxy-filter.txt
...
if [ -f /app/denylist.txt ]; then
  ...
  ' /app/denylist.txt >> /app/tinyproxy-filter.txt
fi
...
if [ -f /app/hosts ]; then
  ...
  ' /app/hosts >> /app/tinyproxy-filter.txt
fi

wc -l /app/tinyproxy-filter.txt
