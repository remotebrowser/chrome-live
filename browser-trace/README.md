...
Every application record is appended as one JSON object per line to `EVENT_LOG_PATH`. `GET /logs` returns that history, including CDP events, tinyproxy lines, recording lifecycle messages, configuration changes, and connection errors. Records include `timestamp`, `level`, and `message`; CDP and tinyproxy logs retain their structured fields such as `event`, `tab_id`, and `tinyproxy_level`.
