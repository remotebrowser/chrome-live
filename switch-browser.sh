#!/bin/sh
# switch-browser chrome|cloak|clearcote
#
...
usage() {
  echo "usage: switch-browser chrome|cloak|clearcote" >&2
  exit 2
}
...
case "$1" in
  ...
  cloak)
    ...
    ;;
  clearcote)
    command -v clearcote-browser >/dev/null 2>&1 || {
      echo "switch-browser: Clearcote not available (amd64 only)" >&2
      exit 1
    }
    ;;
  *) usage ;;
